...
int decrease_key(int position, PriorityQueue queue, ElementType new_key, struct Node *node_list, int *position_to_nodenum) {
    ...
    ElementType temp;
    if(new_key >= queue->element[position]){
        return position;
    }
    else{
...
//    int initial_node = rand()%node_total +1;
    int initial_node = 1;
    struct Node node_list[node_total];
    ...
    temp_edge = node_list[initial_node].next;
    ...
    while (current_add_node_num <= node_total){
        ...
        while(temp_edge != NULL){
            if(node_list[temp_edge->node_num].add_flag == 0){  // for node has not been added to heap
            ...
            else{                                                            // for node has already been added to heap
                position = node_list[temp_edge->node_num].position_in_heap; // get position in heap
...
//    for (int k = 0; k <=node_total ; ++k) {
//        printf("%d:%d\n",k,node_list[k].position_in_heap);
//    }
    printf("mst weight = %d\n", mst_weight);
